<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Menu de Solicitudes</title>
    <link rel="stylesheet" href="css/i.css">
    
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    
</head>
<body>
    
    <div class= "header"> 
    <h1 class="title">Sistema de Solicitudes de Servicio</h1>
    </div>

    <div class="container">
       
        <div class="card">
            <img src="image/PC.jpg">
            <h4>Equipo de Computo</h4>
            <p><li>No enciende el equipo</li>
            <li>Ruidos extraños</li></p>
            <a href="Solicitud.php">Solicitar</a>
        </div>
        
        <div class="card">
            <img src="image/SII.png">
            <h4>SII</h4>
            <p><li>Problemas de inicio sesion</li>
            <li>Servidor caido</li>
            <li>No carga</li></p>
            <a href="Solicitud.php">Solicitar</a>
        </div>
        
        <div class="card">
            <img src="image/SIGNO.png">
            <h4>Internet</h4>
            <p><li>El equipo no se conecta a internet</li>
            <li>no funciona</li></p>
            <a href="Solicitud.php">Solicitar</a>
            
        </div>
        <div class="card">
            <img src="image/SIGNO.png">
            <h4>Impresora</h4>
            <p><li>Se atoro el papel</li>
            <li>No enciende</li>
            <li>No funciona</li</p>
            <a href="Solicitud.php">Solicitar</a>
            
        </div>
        <div class="card">
            <img src="image/SIGNO.png">
            <h4>Telefonia</h4>
            <p><li>No hay linea</li>
            <li>No funciona</li></p>
            <a href="Solicitud.php">Solicitar</a>
            
        </div>
        <div class="card">
            <img src="image/SIGNO.png">
            <h4>Otro</h4>
            <p>
            <textarea class="textarea"Style="resize:none" >Detalles</textarea></p>
            <a href="Solicitud.php">Solicitar</a>
            
        </div>
        
    </div>
    
</body>
</html>